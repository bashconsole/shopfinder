...
class Uninstall implements UninstallInterface
{
    ...
    {
        //remove tables
        if ($setup->tableExists('bashconsole_shopfinder_stores')) {
            $setup->getConnection()->dropTable('bashconsole_shopfinder_stores');
        }
        //remove config settings if any
        $collection = $this->collectionFactory->create()
            ->addPathFilter('bashconsole_shopfinder_stores');
